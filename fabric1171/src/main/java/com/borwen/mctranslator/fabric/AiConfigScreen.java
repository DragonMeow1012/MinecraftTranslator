...
public final class AiConfigScreen extends Screen {
    ...
    protected void init() {
        ...
        baseUrlBox = new EditBox(this.font, x, Y_URL, FIELD_W, 20, new net.minecraft.network.chat.TextComponent("Base URL"));
        baseUrlBox.setMaxLength(256);
        baseUrlBox.setValue(cfg.aiBaseUrl == null ? "" : cfg.aiBaseUrl);
        addRenderableWidget(baseUrlBox);

        modelBox = new EditBox(this.font, x, Y_MODEL, FIELD_W, 20, new net.minecraft.network.chat.TextComponent("Model"));
        modelBox.setMaxLength(128);
        modelBox.setValue(cfg.aiModel == null ? "" : cfg.aiModel);
        addRenderableWidget(modelBox);

        keysBox = new EditBox(this.font, x, Y_KEYS, FIELD_W, 20, new net.minecraft.network.chat.TextComponent("API Keys"));
        ...
        addRenderableWidget(com.borwen.mctranslator.fabric.LegacyButton.builder(new net.minecraft.network.chat.TranslatableComponent("screen.mctranslator.ai.test"), b -> {
            testResult = new net.minecraft.network.chat.TranslatableComponent("screen.mctranslator.ai.testing").getString();
        ...
        addRenderableWidget(com.borwen.mctranslator.fabric.LegacyButton.builder(new net.minecraft.network.chat.TranslatableComponent("gui.done"), b -> onClose())
                .bounds(this.width / 2 - 100, Y_DONE, 200, 20).build());
    }

    private void addPreset(String label, int x, int y, int w, String url, String model) {
        addRenderableWidget(com.borwen.mctranslator.fabric.LegacyButton.builder(new net.minecraft.network.chat.TextComponent(label), b -> {
            TranslatorConfig cfg = MctranslatorFabric.config();
    ...
    public void render(PoseStack g, int mouseX, int mouseY, float partialTick) {
        ...
        this.font.draw(g, new net.minecraft.network.chat.TranslatableComponent("screen.mctranslator.ai.endpoint"), lx, Y_URL - 11, 0xA0A0A0);
        this.font.draw(g, new net.minecraft.network.chat.TranslatableComponent("screen.mctranslator.ai.model"), lx, Y_MODEL - 11, 0xA0A0A0);
        this.font.draw(g, new net.minecraft.network.chat.TranslatableComponent("screen.mctranslator.ai.keys"), lx, Y_KEYS - 11, 0xA0A0A0);
        this.font.draw(g, new net.minecraft.network.chat.TranslatableComponent("screen.mctranslator.ai.presets"), lx, Y_PRESETS - 11, 0xA0A0A0);
        ...
        String[] hints = {
                new net.minecraft.network.chat.TranslatableComponent("screen.mctranslator.ai.hint.gemini").getString(),
                new net.minecraft.network.chat.TranslatableComponent("screen.mctranslator.ai.hint.providers").getString(),
                new net.minecraft.network.chat.TranslatableComponent("screen.mctranslator.ai.hint.enable").getString(),
        };
        g.pushPose();
