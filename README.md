...
AI 精翻不在上述機翻清單內。它使用使用者設定的 OpenAI 相容端點與模型；可搭配 Gemini、OpenAI、DeepSeek、OpenRouter、Ollama、LM Studio、相容的自架服務或其他 OpenAI 相容服務。AI 設定提供「自訂／本地」快速選項，API Key 為選填；留空時不會送出 `Authorization` header。實際支援能力、配額與費用依端點供應者而定。
...
- AI Base URL、模型、選填 API Key、詞彙表與「停用 GT 回退」；自訂／本地預設會帶入 Ollama 常用的 `http://127.0.0.1:11434/v1`，LM Studio 通常可改為 `http://127.0.0.1:1234/v1`。
- 翻譯偵錯浮窗、目前來源／語言快取清除與快捷鍵設定。

為避免把 Minecraft 已本地化的設定文字再送去翻譯，現代版本會整頁略過原版語言、外觀、音效、按鍵／控制、聊天、資源包與協助工具等設定頁。原版顯示／視訊設定仍允許翻譯；未知或模組新增的設定頁預設也允許，因此不會因白名單漏掉第三方選項。
...
新預設是 **10000 ms**；舊版仍使用 6000 ms 預設且未自行改過的設定會一次遷移。AI 與機翻各自計時；`0` 只代表關閉事前冷卻，不會關閉 5 秒批次收集。Google 表示 Gemini 實際 RPM／TPM／RPD 會依模型、專案與帳號狀態變動，應以 AI Studio 顯示的有效限制為準；10 秒是針對免費 Gemini 3.1 Flash-Lite 與游標插隊保留餘裕的保守預設，不能保證不受帳號、每日配額或 IP 限制。失敗項目會受退避與可見性需求控制，避免每個畫面幀都重送。

偵錯浮窗的失敗列會顯示 `failed (原因)`；可辨識 429 rate limit、HTTP 5xx、驗證失敗、逾時／網路、錨點／順序損壞、段落遺失、格式／保護 token 遺失與空回覆。無法可靠分類時才顯示 `unknown`，429 另以醒目顏色標示。
...
- `requestCooldownMs`：新預設 `10000`；舊預設 `6000` 會一次遷移。
- AI 端點、模型、選填 API Key、是否停用 GT 回退及偵錯選項；無金鑰的本地 OpenAI 相容服務可直接使用。
