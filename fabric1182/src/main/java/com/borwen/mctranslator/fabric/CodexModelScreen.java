...
public final class CodexModelScreen extends OptionsSubScreen {
    ...
    public CodexModelScreen(Screen parent) {
        super(parent, Minecraft.getInstance().options,
                new net.minecraft.network.chat.TranslatableComponent("screen.mctranslator.ai.codex.model_picker.title"));
    }

    @Override protected void init() {
        this.modelList = new ModelSelectionList(this.minecraft);
        this.addWidget(this.modelList);
        this.search = new EditBox(this.font, this.width / 2 - 120, 24, 240, 20, new net.minecraft.network.chat.TextComponent(""));
        this.search.setResponder(this.modelList::filterEntries);
        this.addRenderableWidget(this.search);
        this.addRenderableWidget(LegacyButton.builder(CommonComponents.GUI_DONE, button -> onDone())
    ...
    private final class ModelSelectionList extends ObjectSelectionList<ModelSelectionList.Entry> {
        ...
        private final class Entry extends ObjectSelectionList.Entry<Entry> {
            ...
            private Entry(ModelOption option) {
                this.option = option;
                this.label = new net.minecraft.network.chat.TextComponent(option.displayName().equals(option.model())
            ...
            @Override public void render(PoseStack graphics, int index, int y, int x, int width,
                ...
                GuiComponent.drawCenteredString(graphics, CodexModelScreen.this.font, this.label,
            ...
            @Override public Component getNarration() {
                return new net.minecraft.network.chat.TranslatableComponent("narrator.select", this.label);
            }
        }
    }
}