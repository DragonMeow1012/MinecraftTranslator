...
public final class FabricTextStyle {
    ...
    public static Component resolveLegacyCodes(Component source) {
        ...
        source.visit((base, value) -> {
            ...
            for (int i = 0; i < value.length(); i++) {
                ...
                if (!chunk.isEmpty()) {
                    out.append(new net.minecraft.network.chat.TextComponent(chunk.toString()).setStyle(current));
                    chunk.setLength(0);
                }
                current = applyLegacyCode(current, base,
                        Character.toLowerCase(value.charAt(++i)));
            }
            if (!chunk.isEmpty()) out.append(new net.minecraft.network.chat.TextComponent(chunk.toString()).setStyle(current));
    ...
    public static Component renderTranslated(String surfaceId, Component source,
        ...
        if (rendered.mode() == DisplayMode.BOTH && !"tooltip".equals(surfaceId)) {
            return STACKABLE.contains(surfaceId)
                    ...
                    : source.copy().append(new net.minecraft.network.chat.TextComponent("　")).append(translated);
        }
        return translated;
    }
    ...
    private static Rendered translateParagraphBlock(
        ...
        for (int[] range : paragraphRanges(lines)) {
            if (!firstOutput) out.append(new net.minecraft.network.chat.TextComponent("\n"));
            firstOutput = false;
    ...
    public static MutableComponent joinLines(List<Component> group) {
        ...
        for (int i = 0; i < group.size(); i++) {
            if (i > 0) out.append(new net.minecraft.network.chat.TextComponent(" "));
            for (Seg seg : segments(group.get(i))) {
                out.append(new net.minecraft.network.chat.TextComponent(seg.text()).setStyle(seg.style()));
            }
        }
        return out;
    }
    ...
    public static MutableComponent toComponent(FormattedText line) {
        ...
        line.visit((style, str) -> {
            if (!str.isEmpty()) out.append(new net.minecraft.network.chat.TextComponent(str).setStyle(style));
            return Optional.empty();
        }, Style.EMPTY);
        return out;
    }
    ...
    public static MutableComponent toComponent(FormattedCharSequence line) {
        ...
        if (line != null) line.accept((index, style, codePoint) -> {
            out.append(new net.minecraft.network.chat.TextComponent(new String(Character.toChars(codePoint))).setStyle(style));
            return true;
        });
        return out;
    }
        ...
        for (int i = 0; i < paragraph.size(); i++) {
            ...
            if (i > 0) {
                if (continuesSentence(prevPlain, plain)) {
                    out.append(new net.minecraft.network.chat.TextComponent(" "));
                } else {
                    out.append(new net.minecraft.network.chat.TextComponent(ParagraphModel.breakToken(breaks++)));
                }
            }
            if (line != null) {
    ...
    private static void appendStyledRange(
        ...
        for (Seg run : runs) {
            ...
            if (from < to) {
                out.append(new net.minecraft.network.chat.TextComponent(run.text().substring(
    ...
    public static MutableComponent styled(String translated, ColorProfile profile) {
        ...
        return new net.minecraft.network.chat.TextComponent(translated).setStyle(style);
    }
    ...
    public static MutableComponent markedChat(Component original, int contentStart,
        ...
        if (translated == null || translated.isEmpty()) return new net.minecraft.network.chat.TextComponent("");
        if (marked == null || !marked.marked()) {
            ...
            return new net.minecraft.network.chat.TextComponent(stripMarkers(translated)).setStyle(style);
        }
        ...
        while (matcher.find()) {
            if (matcher.start() > pos) {
                ...
                if (!chunk.isEmpty()) out.append(new net.minecraft.network.chat.TextComponent(chunk).setStyle(current == Style.EMPTY ? last : current));
            }
            saw = true;
    ...
    private static MutableComponent rebuildRichSingle(Component original, String translated,
        ...
        if (segs.size() == 1) {
            return new net.minecraft.network.chat.TextComponent(translated).setStyle(segs.get(0).style());
        }
    ...
    public static List<Component> splitStyledLines(Component source) {
        ...
        source.visit((style, value) -> {
            ...
            for (int i = 0; i < value.length(); i++) {
                ...
                if (i > start) {
                    ((MutableComponent) lines.get(lines.size() - 1))
                            .append(new net.minecraft.network.chat.TextComponent(value.substring(start, i)).setStyle(style));
                }
                lines.add(new net.minecraft.network.chat.TextComponent(""));
                start = i + 1;
            }
            if (start < value.length()) {
                ((MutableComponent) lines.get(lines.size() - 1))
                        .append(new net.minecraft.network.chat.TextComponent(value.substring(start)).setStyle(style));
            }
            return Optional.empty();
        }, Style.EMPTY);
        return lines;
    ...
    public static ChatLinePlan prepareChatLine(Component source) {
        Component resolved = resolveLegacyCodes(source == null ? new net.minecraft.network.chat.TextComponent("") : source);
    ...
    public static Component rebuildChatLine(ChatLinePlan plan, String translated) {
        if (plan == null || translated == null) {
            return plan == null ? new net.minecraft.network.chat.TextComponent("") : plan.source().copy();
        }
        ...
        return new net.minecraft.network.chat.TextComponent("").append(takePrefix(source, plan.contentStart())).append(core);
    }
    ...
    public static MutableComponent styledChatContent(Component original, int contentStart, String translated) {
        ...
        if (core.isEmpty()) return new net.minecraft.network.chat.TextComponent(translated).setStyle(segs.get(0).style());
        Style dominant = dominantStyle(segs, 0, segs.size() - 1);
        if (dominant == null) return styled(translated, profile);
        MutableComponent out = new net.minecraft.network.chat.TextComponent("");
        if (lead > 0) out.append(new net.minecraft.network.chat.TextComponent(translated.substring(0, lead)).setStyle(segs.get(0).style()));
        out.append(new net.minecraft.network.chat.TextComponent(core).setStyle(dominant));
        if (trail < translated.length()) {
            out.append(new net.minecraft.network.chat.TextComponent(translated.substring(trail)).setStyle(segs.get(segs.size() - 1).style()));
        }
        return out;
    }
    ...
    public static MutableComponent styledAnchored(Component original, int fromChar, String translated) {
        ...
        for (StyleAnchor anchor : anchors) {
            if (anchor.start() > cursor) {
                out.append(new net.minecraft.network.chat.TextComponent(translated.substring(cursor, anchor.start()))
                        .setStyle(gapStyle(segs, previous, anchor)));
            }
            out.append(new net.minecraft.network.chat.TextComponent(translated.substring(anchor.start(), anchor.end()))
                    .setStyle(segs.get(anchor.segment()).style()));
            cursor = anchor.end();
            previous = anchor;
        }
        if (cursor < translated.length()) {
            out.append(new net.minecraft.network.chat.TextComponent(translated.substring(cursor))
    ...
    private static void appendStyledSlice(MutableComponent out, Component source,
        ...
        source.visit((style, value) -> {
            ...
            if (takeStart < takeEnd) {
                out.append(new net.minecraft.network.chat.TextComponent(value.substring(
    ...
    private static void appendWeighted(MutableComponent out, String text, List<Seg> segs, int from, int to) {
        ...
        if (from > to) {
            ...
            out.append(new net.minecraft.network.chat.TextComponent(text).setStyle(segs.get(idx).style()));
            return;
        }
        int total = 0;
        ...
        if (total <= 0) {
            out.append(new net.minecraft.network.chat.TextComponent(text).setStyle(segs.get(from).style()));
            return;
        }
        int start = 0;
        int cumulative = 0;
        ...
        for (int k = 0; k < weights.length; k++) {
            ...
            if (end > start) {
                out.append(new net.minecraft.network.chat.TextComponent(text.substring(start, end)).setStyle(segs.get(from + k).style()));
                start = end;
            }
        }
        if (start < text.length()) {
            out.append(new net.minecraft.network.chat.TextComponent(text.substring(start)).setStyle(segs.get(to).style()));
        }
    }
    ...
    public static MutableComponent buildColored(List<Seg> segs, List<String> translated) {
        MutableComponent core = new net.minecraft.network.chat.TextComponent("");
        for (int i = 0; i < segs.size(); i++) {
            String t = translated.get(i);
            if (t == null || t.isEmpty()) continue;
            core.append(new net.minecraft.network.chat.TextComponent(t).setStyle(segs.get(i).style()));
        }
        return core;
    }
        ...
        if (c != null) {
            c.visit((style, str) -> {
                ...
                for (int i = 0; i < str.length(); i++) {
                    if (str.charAt(i) == '\n') {
                        if (i > start) {
                            lines.get(lines.size() - 1).append(new net.minecraft.network.chat.TextComponent(str.substring(start, i)).setStyle(style));
                        }
                        lines.add(new net.minecraft.network.chat.TextComponent(""));
                        start = i + 1;
                    }
                ...
                if (start < str.length()) {
                    lines.get(lines.size() - 1).append(new net.minecraft.network.chat.TextComponent(str.substring(start)).setStyle(style));
                }
                return Optional.empty();
            }, Style.EMPTY);
        }
    ...
    public static MutableComponent buildColoredCentered(Font font, List<Seg> segs,
        ...
        for (int i = first; i <= last; i++) {
            ...
            MutableComponent run = new net.minecraft.network.chat.TextComponent(t).setStyle(segs.get(i).style());
            coreWidth += font.width(run);
            core.append(run);
        }

        int spaces = leadSpacesToCenter(font, original, coreWidth);
        return spaces > 0 ? new net.minecraft.network.chat.TextComponent(" ".repeat(spaces)).append(core) : core;
    }
    ...
    public static MutableComponent takePrefix(Component original, int charCount) {
        ...
        original.visit((style, str) -> {
            ...
            if (take > 0) {
                out.append(new net.minecraft.network.chat.TextComponent(str.substring(0, take)).setStyle(style));
            }
            remaining[0] -= take;
            return Optional.empty();
        }, Style.EMPTY);
    ...
    public static MutableComponent separatorLine(int length) {
        ...
        return new net.minecraft.network.chat.TextComponent("-".repeat(n))
    ...
    private static MutableComponent stackAligned(Component original, Component translated) {
        ...
        if (font != null) {
            ...
            if (pad > 0 && w1 < w2) top = original.copy().append(new net.minecraft.network.chat.TextComponent(" ".repeat(pad)));
            else if (pad > 0 && w2 < w1) bottom = translated.copy().append(new net.minecraft.network.chat.TextComponent(" ".repeat(pad)));
        }
        return new net.minecraft.network.chat.TextComponent("").append(top).append(new net.minecraft.network.chat.TextComponent("\n")).append(bottom);
    }
    ...
    private static Component uniqueChatSeparator(int length) {
        ...
        return separatorLine(length).copy().append(new net.minecraft.network.chat.TextComponent(" ".repeat(spaces)));
    }

    public static Component chatBlock(Component original, Component translated) {
        Component source = original == null ? new net.minecraft.network.chat.TextComponent("") : resolveLegacyCodes(original);
