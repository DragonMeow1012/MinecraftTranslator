...
public final class Fabric26TextStyle {
     ...
     *  shred the sentence, so colouring falls back to the proportional stretch instead. */
    private static final int MAX_MARKED_SEGMENTS = 16; // busy broadcasts hit 9-12 runs; gradients are dozens
    ...
    public static MutableComponent markedChat(Component original, int contentStart, String translated, MarkedChat marked) {
        if (translated == null || translated.isEmpty()) return Component.empty();
        if (marked == null || !marked.marked()) return styledAnchored(original, contentStart, stripMarkers(translated));
