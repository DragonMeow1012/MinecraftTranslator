...
final class LegacyAiConfigScreen extends Screen {
    ...
    private void initApi(int x, boolean openAiPanel) {
        ...
        keysBox = new EditBox(font, x, keysY, FIELD_W, 20,
                new TranslatableComponent("screen.mctranslator.ai.keys").getString());
        keysBox.setMaxLength(8000);
        keysBox.setValue(keysForEndpoint(cfg, cfg.aiBaseUrl));
        addButton(keysBox);
        testButton = new Button(x, testY, FIELD_W, 20,
                new TranslatableComponent("screen.mctranslator.ai.test").getString(), button -> {
            saveApiFields();
            button.active = false;
            button.setMessage(new TranslatableComponent("screen.mctranslator.ai.testing").getString());
            LegacyTranslatorMod.testAi(result -> {
                status = result;
                button.active = true;
                button.setMessage(new TranslatableComponent("screen.mctranslator.ai.test").getString());
            });
        });
    ...
    private void initCodex(int x) {
        ...
        Button login = new Button(x, 78, FIELD_W, 20,
                new TranslatableComponent(signedIn ? "screen.mctranslator.ai.codex.logout"
                        : "screen.mctranslator.ai.codex.login").getString(),
        ...
        Button refresh = new Button(x + 258, 104, 62, 20,
                new TranslatableComponent("screen.mctranslator.ai.codex.refresh").getString(),
        ...
        testButton = new Button(x, 156, FIELD_W, 20,
                new TranslatableComponent("screen.mctranslator.ai.test").getString(),
                button -> testCodex());
    ...
    private void testCodex() {
        if (testButton != null) {
            ...
            testButton.setMessage(new TranslatableComponent("screen.mctranslator.ai.testing").getString());
        }
        LegacyTranslatorMod.testAi(result -> {
            status = result;
            if (testButton != null) {
                testButton.active = true;
                testButton.setMessage(new TranslatableComponent("screen.mctranslator.ai.test").getString());
            }
        });
    }
    ...
    private String modelLabel() {
        ...
        return new TranslatableComponent("screen.mctranslator.ai.codex.model", name).getString();
    }
    ...
    private String effortLabel() {
        ...
        return new TranslatableComponent("screen.mctranslator.ai.codex.effort", effort).getString();
    }
    ...
    private void showMissingPrompt() {
        ...
        minecraft.setScreen(new ConfirmScreen(confirmed -> {
            ...
            minecraft.setScreen(this);
                ...
                new TranslatableComponent("screen.mctranslator.ai.codex.open_download").getString(),
    ...
    @Override public void render(int mouseX, int mouseY, float delta) {
        ...
        if (!cfg.aiUseCodex) {
            ...
            font.drawShadow(new TranslatableComponent("screen.mctranslator.ai.endpoint").getString(), x, open ? 72 : 48, 0xA0A0A0);
            font.drawShadow(new TranslatableComponent("screen.mctranslator.ai.model").getString(), x, open ? 106 : 86, 0xA0A0A0);
            font.drawShadow(new TranslatableComponent("screen.mctranslator.ai.keys").getString(), x, open ? 140 : 124, 0xA0A0A0);
        } else {
            LegacyCodexClient client = LegacyTranslatorMod.codexClient();
            LegacyCodexClient.AccountSnapshot account = client == null
                    ? LegacyCodexClient.AccountSnapshot.signedOut() : client.cachedAccount();
